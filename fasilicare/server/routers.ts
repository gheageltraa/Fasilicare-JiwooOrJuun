...
export const appRouter = router({
  ...
  locations: router({ list: publicProcedure.query(() => listLocations()), seed: protectedProcedure.mutation(() => seedLocations()), create: protectedProcedure.input(z.object({ name: z.string().trim().min(2).max(180), type: z.string().trim().max(40).default("Custom") })).mutation(({ input }) => createLocation(input.name, input.type)), unverified: adminProcedure.query(() => listUnverifiedLocations()), verify: adminProcedure.input(z.object({ locationId: z.number().int().positive() })).mutation(({ input }) => verifyLocation(input.locationId, true)), remove: adminProcedure.input(z.object({ locationId: z.number().int().positive() })).mutation(({ input }) => deleteLocation(input.locationId)) }),
  tickets: router({
