...
function buildDemoUser(openId: string, name: string): User {
  ...
  return { id: 0, openId, username: openId, passwordHash: null, name, email: `${openId}@fasilicare.local`, loginMethod: "demo", image: null, reputation: 0, reputationPoints: 0, isBanned: false, role, createdAt: new Date(), updatedAt: new Date(), lastSignedIn: new Date() };
}

function buildGoogleUser(openId: string, name: string, email: string | null, role: User["role"] = "user"): User {
  return { id: 0, openId, username: null, passwordHash: null, name, email, loginMethod: "google", image: null, reputation: 0, reputationPoints: 0, isBanned: false, role, createdAt: new Date(), updatedAt: new Date(), lastSignedIn: new Date() };
}
...
class SDKServer {
  ...
  * Create a session token for an authenticated user openId
  ...
  async authenticateRequest(req: express.Request): Promise<AuthenticatedUser> {
